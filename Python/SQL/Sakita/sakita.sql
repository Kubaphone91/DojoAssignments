...
SELECT customer.first_name, customer.last_name, customer.email, address.address, store.store_id, customer.customer_id
FROM customer
...
SELECT film.title, film.rating, film.special_features, film.description, film.release_year
FROM actor
JOIN film_actor ON film_actor.actor_id = actor.actor_id
JOIN film ON film.film_id = film_actor.film_id
...
SELECT actor.actor_id, film.title, film.film_id, CONCAT(actor.first_name, ' ', actor.last_name) AS actor_name
FROM film
